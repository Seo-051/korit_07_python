...
만약 사용자가 0을 입력하거난 숫자가 아닌 값을 입력하면 적절한 예외 메세지를 출력하시오

지시 사항
1. 사용자로 부터 숫자를 입력받는다
2. 입력 받은 숫바오 100을 나누어 결과 출력
3. 입력값이 0일 경우 예외 처리를 하여 '0으로 나눌 수 없다' 출력
4. 입력값이 숫자가 아닌경우 적절한 예외를 처리하여 '숫자만 입력할 수 있습니다.라는 메시지를 출력한다.
5. 예외가 발생하지 않는 경우 '정상적으로 처리되었습니다.' 출력 결과도 출력
6. 프로그램 종료 메시지 출력
'''
#
# try:
#     num = int(input('숫자를 입력하세요 (0~100): '))
#     print(f'{100/num}')
# except ValueError as e:
#     print('숫자만 입력해야 합니다.', e)
# except ZeroDivisionError as e:
...
'''
1, 사용자 리스트의 인덱스를 입력 받는다.
2. 입력받은 인덱스를 사용하여 리스트의 값을 출력한다
4. 인덱스가 리스트의 범위를 벗어나면 적절한 메시지를 출력한다.
5. 사람을 의심하도 예상되는 예외를 적용한다.
6. 예외가 발생하지 않는 경우 "정상적으로 처리되었습니다." 메세지와 함께 인덱스 값을 출력한다.
7. 프로그램 종료 메세지를 출력한다.
8. 마이너스 인덱스는 적용시키지않는다. -> 사용자 정의 예외 클래스를 통해서 적용합니다.
    -> NegaviceNumIndexError
'''
#
# class NegaviceNumIndexError(ValueError):
#     pass
#
# my_list = [10, 20, 30, 40, 50]
#
# try:
#     index = input('인덱스를 선택해주세여>> ')
#     int(index)
#     if index < 0:
#         print(f'마이너스 인덱스는 적용시키지않는다.{my_list[index]}')
# except ValueError as e:
#     print('숫자만 입력해야 합니다.', e)
# except ZeroDivisionError as e:
#      print('0으로 나눌 수 없다', e)
...
2.사용자로 부터 속성명을 입력 받는다
3. 입력받ㄴ은 '속성명'을 사용하여 객체의 '속성값'을 출력한다.
4. 잘못된 속성명을 입력하면 '존재하지 않는 속성입니다.' 라는 메시지를 출력한다.
5. 예외가 발생하지 않은 경우 '정상적으로 처리되었습니더ㅏ' 라는 메시지와 속성값을 출력한다.
6. 프로그램 종료 메시지를 출력한다,. 
...
print(vars(person1))            # vars(객체명) : 객체의 속성명 - 값을 dictionary로 만등러줍니다. JSON느낌
print(getattr(person1, "age")) # 결과값 : 21
#getattr() 의 두번때 argument는 인스턴스 변수명을 받습니다. -> 그 데이터를 str으로 받습니다.
...
try:
    ...
    print('예측할 수 없는 예외가 발생했ㅅ븐디ㅏ.')
    print(e)
else :
    print('정상적으로 처리 되었습니다.')
    print(f'{attr} , {attr_value}')
finally:
    print('프로그램 종료 메시지를 출력한다')
