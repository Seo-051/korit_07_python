...
'''
# print(scores.pop()) # 근데 .pop()은 call3() 유형입니다. 즉 return값이 있는데, 그 삭제한 element가 return되기 때문에 print(scores.pop())은 현재 scores의 맨마지막 element인 100이 콘솔에 출력 됩니다.
...
'''
    se = {} 형태로 비어있는 set을 생성했을 경우 se는 사실 <class 'dict'>로 나옵니다. 아직 배우지 않은 dictionary 자료형으로 생성됩니다. 그래서 비어있는 set를 생성하기 위해서는 반드시 # 2 방식으로 만들어줘야합니다.
'''
# se2 = set({})       # 이렇게요.
...
# }
'''
    전에 수업했던 것처럼 189번 라인까지 지금 key-value pair가 있는데 189번 라인에 콤마치고 엔터치고 키-값 입력하면 번거로우니까 맨마지막 property에 콤마 찍어주는 게 매너라고 합니다.
