...
'''
#numbers = [ 1, 2, 3, 4, 5]
#chosen_number = random.choice(numbers) # choice()f라는 메서드의 argument로 list를 넣음
# print(chosen_number)
...
print(chosen_word)
# todo - 2 : 사용자에게 알파벳을 하나를 추측해서 입력하라고 요청하고,  이를 guess변수에 담으시오. 그리고 대문자로 시작하는 경우를 방지하기 위해 input() 함수 뒤에 .lower()를 적용하시오
guess = input('알파벳을 하나를 추측해서 입력하세요 >> ').lower()

# todo - 3 : guess에서 입력한 문자 하나가 chosen_word의 string 문자열 중에 하나의 문자와 일치하는지를 반복문을 통해 확인할 수 있도록 프로그램을 작성하시오. 맞으면 정답, 틀리면 오답이라고 출력해야함
